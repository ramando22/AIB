...
###HID_OMNIKEY_CCID_Driver_Installer_x64_WIN10 - required for  NHS SMART CARD (AKA Spine, HSCIC)
Write-Output "##HID_OMNIKEY_CCID_Driver_Installer_x64_WIN10"
cd 'C:\temp\binaries\HID_OMNIKEY_CCID_Driver_Installer'
msiexec /i "HID_OMNIKEY_CCID_Driver_Installer_x64_WIN10_v2.0.0.4.msi" ALLUSERS=1 REBOOT=ReallySuppress /QN
Start-Sleep -s 50
...
& '.\Firefox Setup 91.10.0esr.exe' -ms
...
####Postgress SQL ODBC Drivers (psql ODBC)
Write-Output "##Postgress SQL ODBC Drivers (psql ODBC)"
...
$binary = Get-ChildItem -recurse "C:\temp\binaries\BadgerNet"  -Filter 'BadgerNet Client v3.0.3.msi'
$binaryTransform = Get-ChildItem -recurse "C:\temp\binaries\BadgerNet"  -Filter 'BadgerNet Client v3.0.3.mst'


$BadgerNetbinaryFullName = $binary.FullName
$BadgerNetbinaryTransformFullName = $binaryTransform.FullName

Write-Output $binaryFullName
Write-Output $binaryTransformFullName

$p = Start-Process 'msiexec.exe' -ArgumentList '/i', """$BadgerNetbinaryFullName""", 'TRANSFORMS="C:\temp\binaries\BadgerNet\BadgerNet Client v3.0.3.mst"', "/quiet" -wait
...
####ArcGis
Write-Output "##ArcGis"
cd 'C:\temp\binaries\ArcGis\Package'
.\phegis_install_10_5_1.bat
Start-Sleep -s 50
...
####REPLACE Existing
#### RStudio Desktop####
Write-Output "#### RStudio ####"
$binary = Get-ChildItem "C:\temp\binaries\RStudio"  -Filter *RStudio*.exe*
$binaryFullName = $binary.FullName
echo $binaryFullName
$p = Start-Process $binaryFullName -ArgumentList '/S' -wait
Write-Output $p
...
####REPLACE the Existing with the below
#### RStudio Desktop####
Write-Output "#### RStudio ####"
$binary = Get-ChildItem "C:\temp\binaries\RStudio"  -Filter *RStudio*.exe*
$binaryFullName = $binary.FullName
